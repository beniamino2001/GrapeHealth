...
-- Le regole del decision engine hanno soglie con fonte bibliografica esplicita.
-- "regola_codice" è una FK verso un catalogo di regole, eliminando la
...
-- condizioni simultanee; sunburn: quattro coppie soglia/durata LT50 da Müller and co. 2023).
...
-- tabella di Goidanich (1964) riportata in Pertot, I. and co. (2007).
...
-- oggi ne implementa comunque una sola per regola.
...
CREATE TABLE misurazione (
    ...
    -- da sensors-simulator/simulator/generator.py.
    parametro       VARCHAR(32) NOT NULL
                        CHECK (parametro IN ('temperatura_aria', 'umidita_aria', 'pioggia',
                                              'bagnatura_fogliare', 'psi_stem', 'temperatura_bacca',
                                              'velocita_vento',
...
CREATE TABLE allerta (
    ...
    -- li tiene già allineati, ma questo vincolo dà la stessa sicurezza a
...
-- tipo e parcella_id sono entrambi filtri diretti in AllertaSpecifications (api). Questi due
-- indici sono una precauzione per sessioni sperimentali future molto più lunghe, in
-- assenza di una logica di pulizia periodica del database.
CREATE INDEX idx_allerta_tipo ON allerta (tipo);
CREATE INDEX idx_allerta_parcella ON allerta (parcella_id);
CREATE UNIQUE INDEX idx_allerta_attiva_unica ON allerta (nodo_id, regola_codice) WHERE stato = 'attiva';
...
INSERT INTO regola_soglia (regola_codice, parametro, livello_rischio, operatore, valore_soglia, unita_misura, durata_minima_minuti, note) VALUES
    ...
    ('ondata_di_calore', 'temperatura_aria', 'severo', '>=', 40.0, 'C', NULL, 'Convenzionale, da tessuto fogliare su V. amurensis, Luo and co. 2011'),
    ...
    ('sunburn', 'temperatura_bacca', 'severo',   '>=', 53.79, 'C', 15, 'Dose letale (LT50), Müller and co. 2023'),
    ('sunburn', 'temperatura_bacca', 'severo',   '>=', 49.94, 'C', 30, 'Dose letale (LT50), Müller and co. 2023'),
    ('sunburn', 'temperatura_bacca', 'severo',   '>=', 47.82, 'C', 60, 'Dose letale (LT50), Müller and co. 2023'),
    ('sunburn', 'temperatura_bacca', 'severo',   '>=', 47.06, 'C', 90, 'Dose letale (LT50), Müller and co. 2023'),
    
    -- Svernamento oospore: due soglie (minima e massima), unico livello di rischio
    ('svernamento_oospore', 'temperatura_suolo', 'moderato', '>=', 12.0, 'C', NULL, 'Soglia minima di germinazione, Si Ammour and co. 2020'),
    ('svernamento_oospore', 'temperatura_suolo', 'moderato', '<=', 32.0, 'C', NULL, 'Soglia di inibizione, Si Ammour and co. 2020'),
    ...
    ('infezione_secondaria', 'bagnatura_fogliare', 'moderato', '>=', 40.0, '%', 120, 'Soglia di bagnatura convenzionale (non da Brischetto); durata minima 2h all''ottimo, Brischetto and co. 2021'),
    ('infezione_secondaria', 'temperatura_aria', 'moderato', '>=', 4.0, 'C', NULL, 'Temperatura minima per l''infezione, Brischetto and co. 2021'),
    ('infezione_secondaria', 'temperatura_aria', 'moderato', '<=', 30.2, 'C', NULL, 'Temperatura massima per l''infezione, Brischetto and co. 2021'),
    ...
    ('danno_radicale', 'temperatura_suolo', 'severo', '>=', 35.0, 'C', NULL, 'Soglia di danno radicale, Field and co. 2020');
...
INSERT INTO regola_azione (regola_codice, azione_codice, note) VALUES
    ...
    ('ondata_di_calore', 'nebulizzazione',
        'Sistema di nebulizzazione automatico attivato a 35°C su Sangiovese/Montepulciano, Valentini and co. 2024'),
